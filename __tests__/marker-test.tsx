...
it('Marker test', () => {
  ...
  const marker = renderer
    .create(
      <AnimalMarker animal={anim} longitude={longitude} latitude={latitude} />
    )
    .toJSON();
  expect(marker).toMatchSnapshot();
  const marker2 = renderer
    .create(
      <ItemMarker item={item} longitude={longitude} latitude={latitude} />
    )
    .toJSON();
...
it('Animal marker generate test', () => {
  for (let i = 0; i < 10; i++) {
    const { animal, longitude, latitude } = generateRandomAnimalMarker({
      range: 4,
      latitude: 8,
      longitude: 3,
    });
...
it('Item marker generate test', () => {
  for (let i = 0; i < 10; i++) {
    const { item, longitude, latitude } = generateRandomItemMarker({
      range: 4,
      latitude: 8,
      longitude: 3,
    });
